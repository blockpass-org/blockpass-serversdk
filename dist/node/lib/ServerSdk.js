...
class ServerSdk extends EventEmitter {
  ...
  async loginFow({
    ...
    kycRecord = await Promise.resolve(this.createKyc({
      kycProfile,
      kycToken,
      refId
    }));
    ...
    payload.certs = this.certs; // Request upload data
  ...
  async updateDataFlow({
    ...
    if (!kycRecord) throw new Error('Kyc record could not found'); // query kyc profile

    const kycProfile = await this.blockPassProvider.doMatchingData(kycToken);
    if (kycProfile == null) throw new Error('Sync info failed'); // matching existing record
    ...
    if (redirectForm === 'login') {
      const ssoData = await Promise.resolve(this.generateSsoPayload ? this.generateSsoPayload({
        kycProfile,
        kycRecord,
        kycToken,
        payload
  ...
  async queryStatusFlow({
    ...
    const kycStatus = await Promise.resolve(this.queryKycStatus({
      kycRecord
    })); // checking fields
  ...
  async resubmitDataFlow({
    ...
    if (this.onResubmitKycData) {
      payload = await Promise.resolve(this.onResubmitKycData({
        kycProfile,
        kycRecord,
        kycToken,
        payload,
  ...
  async userNotify({
    ...
    const res = await this.blockPassProvider.notifyUser(bpToken, message, action, type);
    return res;
  } // -----------------------------------------------------------------------------------
  ...
  async pullCertPromise({
    ...
    const response = await this.blockPassProvider.pullCertPromise(bpToken, certPromiseId);
