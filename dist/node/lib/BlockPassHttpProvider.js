...
class BlockpassHttpProvider {
  ...
  async queryPublicKey(hash) {
    try {
      ...
      const pubKeyResponse = await request.get(_baseUrl + api.PUBKEY_PATH + hash);
  ...
  async queryServiceMetadata() {
    try {
      ...
      const metaDataResponse = await request.get(_baseUrl + api.META_DATA_PATH + this._secretId).set({
  ...
  async doHandShake(code, session_code) {
    try {
      ...
      const handShakeResponse = await request.post(_baseUrl + api.HAND_SHAKE_PATH).send({
        client_id: _clientId,
  ...
  async doMatchingData(handShakeToken) {
    try {
      ...
      const userProfileResponse = await request.get(_baseUrl + api.MATCHING_INFO_PATH).set({
  ...
  async notifyLoginComplete(bpToken, sessionData, extraData) {
    try {
      ...
      const ssoCompleteResponse = await request.post(_baseUrl + api.SSO_COMPETE_PATH).set({
  ...
  async _checkAndRefreshAccessToken(bpToken) {
    try {
      ...
      const refreshTokenResponse = await request.post(_baseUrl + api.REFRESH_TOKEN_PATH).send({
        stoc: access_token,
  ...
  async queryProofOfPath(bpToken, slug_list) {
    ...
    try {
      const ssoQueryPathResponse = await request.post(_baseUrl + api.GET_PROOF_OF_PATH).set({
        Authorization: bpToken.access_token
      }).send({
        slug_list
      });
  ...
  async acceptCertificate(bpToken, cerDocument) {
    ...
    try {
      const putCertResponse = await request.put(_baseUrl + api.CERTIFICATE_ACCEPT_PATH).set({
        Authorization: bpToken.access_token
      }).send(cerDocument);
  ...
  async notifyUser(bpToken, msg, action = '', type = 'info') {
    ...
    try {
      const putCertResponse = await request.post(_baseUrl + api.NOTIFICATION_PATH).set({
        Authorization: bpToken.access_token
      }).send({
        type,
        action,
  ...
  async deactivateUser(bpToken) {
    ...
    try {
      const deactiveResponse = await request.post(_baseUrl + api.DEACTIVE_USER_PATH).set({
        Authorization: bpToken.access_token
      }).send();
  ...
  async fetchCertPromise(bpToken) {
    ...
    try {
      const certPromiseResponse = await request.get(_baseUrl + api.FETCH_CERT_PROMISE_PATH).set({
        Authorization: bpToken.access_token
      });
  ...
  async pullCertPromise(bpToken, certPromiseId) {
    ...
    try {
      const certPromiseResponse = await request.get(_baseUrl + api.PULL_CERT_PROMISE_PATH + '/' + certPromiseId).set({
        Authorization: bpToken.access_token
      });
  ...
  async checkCertificateHash({
    ...
    try {
      const certHashResponse = await request.get(_baseUrl + api.CHECK_CERT_HASH_PATH + '/' + certHash).set({
        Authorization: bpToken.access_token
      });
